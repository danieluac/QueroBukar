    ...
    <div class="section-body">
        ...
        <div class="section-body-right">
            ...
            <div class="form-group col-md-12 ">
                <select id="PutCat" onchange="getCategoria(this.value,this.id)" class="form-control" name="categoriasBook">
                    <option disabled="" selected="">Selecione a categorias:</option>
                 <?php 
                for($a=0;$a<count($this->view["categoria"]);$a++){?>
                    
                    <option value="<?=$this->view["categoria"][$a]->id.":".$this->view["categoria"][$a]->nome?>">
                    <?=$this->view["categoria"][$a]->nome?></option>
            ...
            <form class="" role="form" method="POST" action="<?=$this->route("PostRequest/Publication")?>" enctype="multipart/form-data">
              <div id="getIdCat">
              
              </div>
              <input  type="hidden" name="idusuario" value="<?=$_SESSION["UserId"]?>"/>
            <div class="form-group col-md-12 ">
<input type="text" name="titulo" class="form-control" required placeholder="Titulo"/>
           </div>
            <div class="form-group col-md-12 ">

<textarea rows="3" name="descricao" class="form-control" required placeholder="escreva alguma coisa...">
</textarea>
           </div>
           <div class="form-group col-md-12 ">
               <label for="file">Carregar foto <span>(*opcional)</span></label>
            <input type="file" id="file" name="file[]" class="form-control" placeholder="foto"/>
          </div>
           <div class="form-group col-md-12 ">
             <button type="submit" class="btn btn-primary black-sgb">Publicar</button>
          </div>
        </form>
...
<?php }?>
      ...
      <div class="circle-search">  
       ...
       <a href="<?=$this->route("Studant/profille/".$this->view["contar"]."/".$this->view["cat"])?>" id="vermais" class="">+</a>
         <?php }else{?>
        <a href="<?=$this->route("Studant/profille/".$this->view["contar"])?>" id="vermais" class="">+</a>
         <?php }?>
    </div>
...
{
    $("#getIdCat").empty().append('<input type="hidden" name="idcategoria" value="'+value.split(":")[0]+'"/>');
    $("#titleCat").empty().append("@"+value.split(":")[1]);
}
function getCategorys(value)
    {
        location.href=window.location.origin+"/Studant/profille/0/"+value;
    }
</script>
