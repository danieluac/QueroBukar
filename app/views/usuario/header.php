...
<body>
    ...
    <div class="header-header" style="background:url('<?=$this->asset("icones/1.jpg")?>');">
      <div class=" resp-img" >
         ...
         <img src="<?= $this->route($this->view["userFoto"])?>">
          ...
          <?php }?>
         <form method="post" id="FormUserPhone" action="<?=$this->route("PostRequest/UserFile")?>" enctype="multipart/form-data">
             <label for="fileUser">
           <img src="<?= $this->asset('icones/camara.png')?>">
           <input id="fileUser" type="file" name="fileUser[]" />
        </label>
    ...
    <script>
        ...
        {
            setTimeout(function(){$("#FormUserPhone").submit();},1500);
